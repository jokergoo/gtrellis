...
\arguments{
  ...
  \item{data}{a data frame with at least three columns. The first three columns should be genomic categories (e.g. chromosomes), start positions and end positions. This data frame is used to extract ranges for each genomic category (minimaland maximal positions are taken as the range in the corresponding category).}
  \item{category}{subset of categories. It is also used for ordering.}
  \item{species}{Abbreviations of species. e.g. hg19 for human, mm10 for mouse. If thisvalue is specified, the function will download \code{chromInfo.txt.gz} fromUCSC ftp automatically. Short scaffolds will be removed if they have obvious different length as others. The argument is passedto \code{\link[circlize]{read.chromInfo}}.}
  ...
  \item{track_ylim}{ranges on y axes of tracks. The value can be a vector of length two which means all tracks share samey ranges, or a matrix with two columns, or a vector of length \code{2*n_track} which will be coercedinto the two-column matrix by rows.}
  ...
  \item{equal_width}{whether all columns in the layout have the same width. If \code{TRUE}, short categories will be extendedaccording to the longest category.}
  ...
  \item{xpadding}{padding on x axes in each cell. Numeric value means relative ratio corresponding to the cell width. Use \code{\link[base]{I}} to set it as absolute value which is measured in the data viewport (the coordinate system correspondingto the real data). Currently you cannot set it as a \code{\link[grid]{unit}} object.}
  \item{ypadding}{padding on y axes in each cell. Only numeric value is allowed currently.}
  \item{gap}{0 or a \code{\link[grid]{unit}} object. If it is length two, the first element corresponds to the gaps between rows andthe second corresponds to the gaps between columns.}
  ...
  \item{add_name_track}{whether add a pre-defined name track (insert before the first track). The name track is simply a trackwhich only contains text. The default style of the name track is simple, but users canself define their own by \code{\link{add_track}}.}
  ...
  \item{add_ideogram_track}{whether to add a pre-defined ideogram track (insert after the last track). If the cytoband data for specifiedspecies is not available, this argument is ignored. The ideogram track simply contains rectangleswith different colors, implemented by \code{\link{add_track}}.}
  ...
  \item{legend}{a \code{\link[grid]{grob}} object}
