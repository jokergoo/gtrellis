...
initialize_layout(n_track = 3, ncol = 4, border = FALSE, xaxis = FALSE, track_axis = FALSE, xlab = "")
add_cell_info()
```
...
```{r, fig.width = 8, fig.height = 8}
initialize_layout(nrow = 5, byrow = FALSE, track_ylim = range(bed[[4]]))
add_track(bed, panel.fun = function(bed) {
    x = (bed[[2]] + bed[[3]]) / 2
    y = bed[[4]]
    grid.points(x, y, pch = 16, size = unit(0.5, "mm"))
...
initialize_layout(n_track = 4, ncol = 4, byrow = FALSE,
...
initialize_layout(n_track = 2, ncol = 4, byrow = FALSE,
    track_axis = TRUE, 
...
in any specified chromosomes and tracks by specifying `category` and `i_track`.
...
initialize_layout(nrow = 5, byrow = FALSE, track_ylim = range(bed[[4]]), newpage = FALSE)
add_track(bed, panel.fun = function(bed) {
    x = (bed[[2]] + bed[[3]]) / 2
    y = bed[[4]]
    grid.points(x, y, pch = 16, size = unit(0.5, "mm"))
...
initialize_layout(byrow = FALSE, n_track = 2, ncol = 4, 
...
`i_category` and `i_track`. Pseudo code are:
...
xlim = get_cell_meta_data("xlim", i_category = "chr2", i_track = 1)
```
