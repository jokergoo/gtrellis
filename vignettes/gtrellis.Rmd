...
it can easily reveal multirelationship  For genomic data, chromosomes are always 
...
- **ggbio** also supports trellis visualizatioin for the whole genome, but still,
  only one track is supported. (Only for continuous region, multiple tracks are supported
  in **ggbio**. Anyway, you can use `grid.layout` to arrange multiple continuous regions,
  but the solution is not so straightforward.) 

Here, **gtrellis** aims to arrange chromosomes as trellis style and supports multiple
tracks for visualization. **gtrellis** is implemented in **grid** graphic system, so, in order
to add graphics in each panel, you only need to use low-level graphic functions 
(`grid.points`, `grid.lines`, `grid.rect`, ...) which are quite similar as those in 
classic graphic system. There is only one thing you need to take care of is the unit
in **grid** system.
...
`add_cell_info()` here is an asistant function to add the information to each cell.

```{r, fig.width = 6, fig.height = 6}
initialize_layout(category = c("chr3", "chr1"))
add_cell_info()
```
...
you feel the width for some chromosomes are too small. For chromosomes in the same column,
...
```{r, fig.width = 10, fig.height = 6}
initialize_layout(n_track = 3, track_height = c(1, 2, 3))
```
...
Whether showing y-axes by setting `track_axis`. If certain value is set to `FALSE`, 
corresponding track y-axis will not be drawn.

```{r, fig.width = 10, fig.height = 6}
initialize_layout(n_track = 3, track_axis = c(FALSE, TRUE, FALSE), xaxis = FALSE, xlab = "")
```

Set y-lim by `track_ylim`. It should be a two-column matrix, but to make it easy, it can
...
```{r, fig.width = 10, fig.height = 6}
initialize_layout(n_track = 3, track_ylim = c(0, 3, -4, 4, 0, 1000000))
```
...
```{r, fig.width = 10, fig.height = 6}
initialize_layout(n_track = 3, track_ylim = c(0, 3, -4, 4, 0, 1000000), asist_ticks = FALSE)
```

Set x-label by `xlab` and set y-labels by `track_ylab`.

```{r, fig.width = 10, fig.height = 6}
initialize_layout(n_track = 3, main = "main", track_ylab = c("", "bbbbbb", "ccccccc"), xlab = "xlab")
```

Since chromosomes can have more than one tracks, following shows multiple columns and multiple tracks.
...
Set `border` to `FALES` not to show borders.

```{r, fig.width = 8, fig.height = 12}
initialize_layout(n_track = 3, ncol = 4, border = FALSE, xaxis = FALSE, track_axis = FALSE)
add_cell_info()
```

## Add graphics track by track

After the initialization of the layout, each cell can be thought as a coordinate
system. Then graphics can be added in cell by cell afterwards.
...
gr = GRanges(seqnames = bed[[1]],
             ...
             score = bed[[4]])
initialize_layout(track_ylim = range(gr$score))
...
Also, we manually add track which contains chromosome names
...
Initialize the layout and add four tracks which are

- chromosome names: text
- rainfall plot: first make a rainfall transformation and then add points
- genomic density: lines with area (actually it is polygons)
- ideogram: rectangles

```{r, fig.width = 10, fig.height = 12}
initialize_layout(n_track = 4, ncol = 4,
...
`n_track` as 2, but the name track and ideogram track are also added, thus, the
final number of track is 4. Also note how we specify `i_track` argument.
...
initialize_layout(n_track = 2, ncol = 4,
    track_axis = TRUE, 
...
add_track(DMR_hyper, i_track = 2, panel.fun = function(gr) {
    df = rainfallTransform(gr[2:3])
    x = (df[[1]] + df[[2]])/2
    y = log10(df[[3]])
    grid.points(x, y, pch = 16, size = unit(0.5, "mm"), gp = gpar(col = "#FF000040"))
})
add_track(DMR_hypo, i_track = 2, panel.fun = function(gr) {
    df = rainfallTransform(gr[2:3])
    x = (df[[1]] + df[[2]])/2
    y = log10(df[[3]])
    grid.points(x, y, pch = 16, size = unit(0.5, "mm"), gp = gpar(col = "#0000FF40"))
})
add_track(DMR_hyper_density, i_track = 3, panel.fun = function(gr) {
    x = (gr[[3]] + gr[[2]])/2
    y = gr[[4]]
    grid.polygon(c(x[1], x, x[length(x)]), 
                 c(0, y, 0), default.units = "native", gp = gpar(fill = "#FF000040"))
})
add_track(DMR_hypo_density, i_track = 3, panel.fun = function(gr) {
    x = (gr[[3]] + gr[[2]])/2
    y = gr[[4]]
    grid.polygon(c(x[1], x, x[length(x)]), 
...
in specified chromosomes and tracks by specifying `category` and `i_track`.
...
As a real application, following codes plot coverage for a tumor, its companion normal sample
...
initialize_layout(n_track = 3, nrow = 3, byrow = FALSE, gap = unit(c(4, 1), "mm"),
...
Following figure is karyogram view of genomic regions (reproduce http://www.tengfei.name/ggbio/docs/man/layout_karyogram-method.html). We arrange the layout as 
...
We specified `n_track` to 1, but we also specify `add_ideogram_track`, so actually
there are two tracks. We manually add text in the second track by specifying `i_track`. 

```{r, fig.width = 8, fig.height = 8}
bed = generateRandomBed(nr = 10000)
bed = bed[sample(10000, 100), ]
col_fun = colorRamp2(c(-1, 0, 1), c("green", "yellow", "red"))
initialize_layout(n_track = 1, ncol = 1, track_axis = FALSE, xpadding = c(0.1, 0),
...
library(GetoptLong)
initialize_layout(category = c("chr1", "chr2", "chr21", "chr22"), equal_width = TRUE, add_name_track = TRUE, add_ideogram_track = TRUE, nrow = 2,
...
add_track(panel.fun = function(gr) {
  xlim = get_cell_meta_data("xlim")
    ...
    i_col = get_cell_meta_data("i_col")
    i_row = get_cell_meta_data("i_row")
    i_track = get_cell_meta_data("i_track")
    grid.text(qq("name = @{name}\ni_col = @{i_col}\ni_row = @{i_row}\ni_track = @{i_track}"), 0.5, 0.5)
})
```
...
initialize_layout(data = df, n_track = 1, track_ylim = c(0.5, n+0.5), 
...
df
n = max(sapply(tp_family, length))
initialize_layout(data = df, n_track = 1, ncol = 1, track_ylim = c(0.5, n+0.5), 
...
zoom = function(df) {
	initialize_layout(data = df, n_track = 3, nrow = 2,
    track_ylim = c(cov_range, cov_range, ratio_range),
    track_ylab = c("tumor, log10(cov)", "control, log10(cov)", "ratio, log2(ratio)"), 
    add_name_track = TRUE, add_ideogram_track = TRUE)
	add_track(tumor_df, panel.fun = function(gr) {
...
initialize_layout(df, ncol = 1)
```


```{r, fig.width = 10, fig.height = 6}
gene = readRDS(paste0(system.file(package = "gtrellis"), "/extdata/gencode_v19_protein_coding_genes.rds"))
...
initialize_layout(byrow = FALSE, n_track = 2, ncol = 4, add_ideogram_track = TRUE, 
  add_name_track = TRUE,
...
col_fun = circlize::colorRamp2(seq(0, max(gene_density[[4]]), length = 11), rev(brewer.pal(11, "RdYlBu")))
add_track(gene_density, panel.fun = function(gr) {
