...
is always the chromosome. The advantage of Trellis graph is that
...
For a single region, multiple tracks are supported in **ggbio** and **Gviz**. 
But if you want to compare more than one regions, things will be complex.
Due to the design of **ggbio** or **Gviz**, it will not be so efficient to visualize e.g. 10
regions at a same time.

Here, **gtrellis** provides a flexible way to arrange genomic categories. It has following advantages:

- The layout is flexible that it is good at visualizing many genomic regions at a same time (e.g. 24 human chromosomes).
...
First we will introduce functions which define fixed types of graphics.
...
add_points_track(bed, bed[[4]], gp = gpar(ifelse(bed[[4]] > 0, "red", "green")))
```

`add_segmetns_track()` adds segments for corresponding regions.
...
`add_lines_track()` adds lines. Also it can draw areas below the lines. 
...
`add_heatmap_track()` adds heatmap. Heatmap will fill the whole track.
...
add_points_track(bed, bed[[4]], track = 1, size = unit(abs(bed[[4]]), "mm"))
```
...
DMR_hyper_density = circlize::genomicDensity(DMR_hyper)
...
```{r, fig.width = 10, fig.height = 12}
DMR_hypo_density = circlize::genomicDensity(DMR_hypo)
...
add_graphics = function() {
    ...
    add_lines_track(DMR_hypo_density, DMR_hyper_density[[4]], area = TRUE, track = current_track(),
...
add_heatmap_track(gene_density, fill = col_fun)
```
