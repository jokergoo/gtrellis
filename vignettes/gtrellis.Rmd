...
When there are multiple rows, chromosomes on a same row can be put compactly without aligned to 
...
First we will introduce functions which define a fixed type of graphics.
...
add_lines_track(bed, bed[[4]], track = 1) 
```
...
Graphics are added by the self-defined function `panel_fun`. Similar as **circlize** 
package, `panel_fun` will be applied in every cell in 'the current track'.
The first argument of `add_track()` can be either a `GRanges` object or a data frame,
and the argument in `panel_fun` is a subset of data in current chromosome.
...
add_track(DMR_hyper, panel_fun = function(gr) {
    df = circlize::rainfallTransform(gr[2:3])
    x = (df[[1]] + df[[2]])/2
    y = log10(df[[3]])
    grid.points(x, y, pch = 16, size = unit(1, "mm"), gp = gpar(col = "red"))
})

# track for genomic density
add_track(DMR_hyper_density, panel_fun = function(gr) {
    x = (gr[[3]] + gr[[2]])/2
    y = gr[[4]]
...
add_track(DMR_hyper, track = 2, panel_fun = function(gr) {
    df = rainfallTransform(gr[2:3])
    x = (df[[1]] + df[[2]])/2
    y = log10(df[[3]])
    grid.points(x, y, pch = 16, size = unit(1, "mm"), gp = gpar(col = "#FF000080"))
})
add_track(DMR_hypo, track = 2, panel_fun = function(gr) {
    df = rainfallTransform(gr[2:3])
    x = (df[[1]] + df[[2]])/2
    y = log10(df[[3]])
    grid.points(x, y, pch = 16, size = unit(1, "mm"), gp = gpar(col = "#0000FF80"))
})
add_track(DMR_hyper_density, track = 3, panel_fun = function(gr) {
    x = (gr[[3]] + gr[[2]])/2
    y = gr[[4]]
    grid.polygon(c(x[1], x, x[length(x)]), 
                 c(0, y, 0), default.units = "native", gp = gpar(fill = "#FF000080"))
})
add_track(DMR_hypo_density, track = 3, panel_fun = function(gr) {
    x = (gr[[3]] + gr[[2]])/2
    y = gr[[4]]
    grid.polygon(c(x[1], x, x[length(x)]), 
                 c(0, y, 0), default.units = "native", gp = gpar(fill = "#0000FF80"))
})
```

```{r, fig.width = 14, fig.height = 7}
gtrellis_layout(n_track = 2, nrow = 3, compact = TRUE,
    track_axis = TRUE, 
    track_height = unit.c(unit(1, "null"), 
                          unit(0.5, "null")), 
    track_ylim = c(0, 8, c(0, max(c(DMR_hyper_density[[4]], DMR_hypo_density[[4]])))),
...
add_track(tumor_df, panel_fun = function(gr) {
    x = (gr[[2]] + gr[[3]])/2
    y = gr[[4]]
...
add_track(control_df, panel_fun = function(gr) {
    x = (gr[[2]] + gr[[3]])/2
    y = gr[[4]]
    grid.points(x, y, pch = 16, size = unit(2, "bigpts"), gp = gpar(col = "#00000020"))
})
...
add_track(gene_density, panel_fun = function(gr) {
    x = (gr[[3]] + gr[[2]])/2
    y = gr[[4]]
...
in **ComplexHeatmap** package or `legendGrob()` in grid package can be used to create a legend grob. 
Or you can consider to use `frameGrob()` and `packGrob()` to build a legend from ground.

```{r, fig.width = 8, fig.height = 8}
bed = generateRandomBed()
lgd = legendGrob("label", pch = 16)
